...
#            print(word)
            ...
            if rv.endswith(suffix):
                if suffix in ("v", "vshi", "vshis'"):
                    if (rv[-len(suffix)-3:-len(suffix)] == "i^a" or
                        ...
                        suffixcash.update({'pref_ger_suff' : self.__roman_to_cyrillic(suffix)})
                        suffixlist.append(self.__roman_to_cyrillic(suffix))
                        word = word[:-len(suffix)]
                        r2 = r2[:-len(suffix)]
                        rv = rv[:-len(suffix)]
                        step1_success = True
                        speakcash = speakcash + '\n' + 'Step 1 val = i^a or a + prefective gerund suffix ' + suffix
                        break
                else:
                    word = word[:-len(suffix)]
                    r2 = r2[:-len(suffix)]
                    ...
                    suffixcash.update({'pref_ger_suff' : self.__roman_to_cyrillic(suffix)})
                    suffixlist.append(self.__roman_to_cyrillic(suffix))
                    speakcash = speakcash + '\n' + 'Step 1 val = prefective gerund suffix ' + suffix
                    break
        ...
        if not step1_success:
            for suffix in self.__reflexive_suffixes:
                if rv.endswith(suffix):
                    ...
                    r2 = r2[:-len(suffix)]
                    rv = rv[:-len(suffix)]
                    suffixcash.update({'refl_suff' : self.__roman_to_cyrillic(suffix)})
                    suffixlist.append(self.__roman_to_cyrillic(suffix))
                    speakcash = speakcash + '\n' + 'Step 1 val = reflexive suffix ' + suffix
                    break
                ...
                if rv.endswith(suffix):
                    if suffix in ('i^ushchi^ui^u', 'i^ushchi^ai^a',
                        ...
                        if (rv[-len(suffix)-3:-len(suffix)] == "i^a" or
                            ...
                            suffixcash.update({'adj_suff' : self.__roman_to_cyrillic(suffix)})
                            suffixlist.append(self.__roman_to_cyrillic(suffix))
                            speakcash = speakcash + '\n' + 'Step 1 val = i^a or a + adjectival suffix ' + suffix
                            break
                    else:
                        word = word[:-len(suffix)]
                        r2 = r2[:-len(suffix)]
                        ...
                        suffixcash.update({'adj_suff' : self.__roman_to_cyrillic(suffix)})
                        suffixlist.append(self.__roman_to_cyrillic(suffix))
                        speakcash = speakcash + '\n' + 'Step 1 val = adjectival suffix ' + suffix
                        break
                    ...
                    if rv.endswith(suffix):
                        if suffix in ("la", "na", "ete", "i`te", "li",
                            ...
                            if (rv[-len(suffix)-3:-len(suffix)] == "i^a" or
                                ...
                                rv = rv[:-len(suffix)]
                                verb_removed = True
                                suffixcash.update({'verb_suff' : self.__roman_to_cyrillic(suffix)})
                                suffixlist.append(self.__roman_to_cyrillic(suffix))
                                speakcash = speakcash + '\n' + 'Step 1 val = i^a or a + verbal suffix ' + suffix
                                break
                        else:
                            word = word[:-len(suffix)]
                            r2 = r2[:-len(suffix)]
                            ...
                            suffixcash.update({'verb_suff' : self.__roman_to_cyrillic(suffix)})
                            suffixlist.append(self.__roman_to_cyrillic(suffix))
                            speakcash = speakcash + '\n' + 'Step 1 val = verbal suffix ' + suffix
                            break
            ...
            if not adjectival_removed and not verb_removed:
                for suffix in self.__noun_suffixes:
                    if rv.endswith(suffix):
                        ...
                        r2 = r2[:-len(suffix)]
                        rv = rv[:-len(suffix)]
                        suffixcash.update({'noun_suff' : self.__roman_to_cyrillic(suffix)})
                        suffixlist.append(self.__roman_to_cyrillic(suffix))
                        speakcash = speakcash + '\n' + 'Step 1 val = noun suffix ' + suffix
                        break
        ...
        if rv.endswith("i"):
            word = word[:-1]
            r2 = r2[:-1]
            suffixcash.update({'intermediate' : 'и'})
            suffixlist.append('и')
            speakcash = speakcash + '\n' + 'Step 2 val = intermediate и'

        # Step 3
        for suffix in self.__derivational_suffixes:
            if r2.endswith(suffix):
                word = word[:-len(suffix)]
                suffixcash.update({'deriv_suff' : self.__roman_to_cyrillic(suffix)})
                suffixlist.append(self.__roman_to_cyrillic(suffix))
                speakcash = speakcash + '\n' + 'Step 3 val = derivational suffix ' + suffix
                break
        ...
        if word.endswith("nn"):
            ...
            suffixcash.update({'nn_suff' : 'нн'})
            suffixlist.append('нн')
            speakcash = speakcash + '\n' + 'Step 4 val = intermediate НН'
        ...
        if not undouble_success:
            for suffix in self.__superlative_suffixes:
                if word.endswith(suffix):
                    ...
                    suffixcash.update({'superl_suffix ' : self.__roman_to_cyrillic(suffix)})
                    suffixlist.append(self.__roman_to_cyrillic(suffix))
                    speakcash = speakcash + '\n' + 'Step 4 val = superlative suffix '+ suffix 
                    break
            if word.endswith("nn"):
                word = word[:-1]
                suffixcash.update({'nn_suff' : 'нн'})
        ...
        returncash.update(suffixcash)

        rlist = list(returncash.values())
#        print(rlist)
#        print(type(rlist))
        rlist = list(reversed(rlist))
        
        suffixlist = list(reversed(suffixlist))
#        print(rlist)
#        print(type(rlist))
        self.suffixcash = suffixcash
        self.suffixlist = suffixlist
        suffixlist.append('')
        return [word+':'+ ':'.join(rlist), word, suffixlist, returncash, speakcash]
    ...
    def gtprst(self):
        prst = ('в', 'во', 'взо', 'вы', 'до', 'за', 'из', 'изо', 'на', 'над',
                ...
                'долго', 'ре', 'много', 'само', 'ино', 'объ','дис','анти')
    ...
    def gtpost(self):
        postfix = ('ъ', 'а', 'я', 'о', 'е', 'ь', 'и', 'ы', 'а', 'ая', 'яя', 'ое', 'ее', 'ый', 'ать', 
               'ять', 'еть', 'уть', 'у', 'ю', 'ем', 'ешь', 'ете', 'ет', 'ут', 'ют', 'ал', 'ял', 'ала',
               'яла', 'али', 'яли', 'ул', 'ула', 'ули', 'а', 'ам', 'ами', 'ас', 'am', 'ax', 'ая', 'е', 
               'её', 'ей', 'ем', 'еми', 'емя', 'ex', 'ею', 'ёт', 'ёте', 'ёх', 'ёшь', 'и', 'ие', 'ий',
               'им', 'ими', 'ит', 'ите', 'их', 'ишь', 'ию', 'ыми', 'ми', 'мя', 'о', 'ов', 'ого', 'ость',
               'ое', 'оё', 'ой', 'ом', 'ому', 'ою', 'cm', 'у', 'ум', 'умя', 'ут', 'ух', 'ую', 'шь', 'оть', 'ия',
               'ить', 'ев','ые','ым','ят', 'ых')
        return(postfix)
    ...
    def gtsfx(self):
        ...
        mysfx = ('ятин', 'ёшк', 'атур', 'от', 'ева', 'арх', 'ельник', 'я', 'ийск', 
                 ...
                 'енц', 'ни', 'щик', 'ствов', 'анци', 'ня', 'ческ', 'ою', 'анёк', 
                 ...
                 'чиц', 'венн', 'ильщик', 'нй', 'ичк', 'ейш', 'ёноч', 'евш')
        return(mysfx)
